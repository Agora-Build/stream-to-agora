...
/// Map ffprobe's codec name to the shim's `codec_type`. H.264 → 0, which
/// makes the shim keep `SenderOptions` at the C++ default (the proven
/// v0.2.2 H.264 path) and the per-frame `info.codecType` fall back to
/// H264. HEVC → 3 (H265), which also pins the track's encoder config.
